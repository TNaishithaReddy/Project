...
class Logger:
    ...
    def log_parameter(self, param_name, param_value):
        ...
        entry_data = [0,param_name,"parameter",param_value]
        entry_index = ["step","name","type","value"]
        new_entry = pd.DataFrame(entry_data,entry_index)
...
#        self.logs = self.logs.append(entry, ignore_index=True)
    ...
    def log_metric(self, metric_name, value, step=None):
        ...
        entry_data = [step,metric_name,"metric",value]
        entry_index = ["step","name","type","value"]
        new_entry = pd.DataFrame(entry_data,entry_index)
        self.logs= pd.concat([self.logs,new_entry],ignore_index=True)
        #self.logs = self.logs.append(entry, ignore_index=True)
