...
class Agent:
    ...
    def __init__(self, network, state_size, action_size, config=Config(), seed=42, save=True, save_loc='models/', save_every=2, checkpoint_file=None):
       ...
       # self.sess = tf.Session()      
        ...
        self.act_op = tf1.cond(tf1.random_uniform([1], dtype=tf.float32)[0] > self.eps,
                              lambda: tf.argmax(
                                  self.qnetwork_local.output, output_type=tf.int32, axis=1),
                              lambda: tf1.random_uniform([1], minval=0, maxval=action_size, dtype=tf.int32))
    ...
    def _get_soft_update_op(self):
        Qvars = tf1.get_collection(
            tf1.GraphKeys.TRAINABLE_VARIABLES, scope='inference_local')
        target_vars = tf1.get_collection(
            tf1.GraphKeys.TRAINABLE_VARIABLES, scope='inference_target')
